...
class Connections(WMLResource):
    ...
    def get_details(self, connection_id=None):
        ...
        **Example**
        ...
        if connection_id:
            response = requests.get(self._href_definitions.get_connection_by_id_href(connection_id),
            ...
            if response.status_code == 200:
                return {
                    'resources': [self._get_required_element_from_response(x) for x in response.json()['resources']]}
            else:
                return self._handle_response(200, u'get connection details', response)
